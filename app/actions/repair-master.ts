...
import { db } from "@/db"
import { repairMasterItems, repairMasterSites } from "@/db/schema/repair-master"
import {
...
const REPAIR_MASTER_PATH = "/dashboard/repair-retread/master-barang-repair"

const repairMasterItemSchema = z.object({
...
const repairMasterSiteSchema = z.object({
...
async function getStockSapRows(materialCodes: string[]) {
  ...
  try {
    ...
    return stockSapResult.rows as Array<{
    ...
    console.warn("Repair master stock enrichment skipped", error)
    return []
  }
}
...
export async function getRepairMasterData() {
  ...
  const stockRows = await getStockSapRows(materialCodes)

  return {
    items: mergeRepairMasterItemsWithStockSap(items, stockRows),
    sites,
  }
}
...
export async function upsertRepairMasterItem(data: z.infer<typeof repairMasterItemSchema>, id?: number) {
  try {
    ...
    revalidatePath("/dashboard/repair-retread/wip-repair")
    return { success: true }
  } catch (error) {
