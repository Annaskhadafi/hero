...
import { legacyDb as db } from "@/db/legacy"
...
export async function getA2RCompetitionFilterOptions() {
    await getAuthenticatedSession("marketing_a2r_competition", "view")
...
export async function saveA2RCompetitionTargets(period: string, items: z.infer<typeof a2rSalesTargetItemSchema>[]) {
    await getAuthenticatedSession("marketing_a2r_competition", "edit")
    await ensureA2RSalesTargetsTable()
...
export async function getA2RCompetitionData(rawFilters: z.input<typeof a2rCompetitionFiltersSchema>) {
    ...
    const [salesResult, slowMovingResult, cosmeticMatchResult, consignmentMatchResult, targetResult, materialCategoryResult] = await Promise.all([
        ...
        db.execute(sql`
            SELECT DISTINCT
                ...
                COALESCE(ct.serial_number, '') AS "serialNumber"
            FROM deliveries d
    ...
    // SN lookup: materialKey -> set of serial numbers matched from cosmetic_tires
    const cosmeticSnByMaterial = new Map<string, Set<string>>()

    for (const row of cosmeticMatchResult.rows as Array<{ doSap: string; deliveryNumber: string; materialNumber: string; serialNumber: string }>) {
        ...
        if (materialKey && row.serialNumber) {
            const snSet = cosmeticSnByMaterial.get(materialKey) || new Set<string>()
            snSet.add(row.serialNumber)
            cosmeticSnByMaterial.set(materialKey, snSet)
        }
    }

    // Build consignment match set (customer + material pairs that have cosmetic tires)
    for (const row of consignmentMatchResult.rows as Array<{ customerName: string; materialNumber: string; serialNumber: string }>) {
    ...
    const ensureSoldProduct = (
        ...
        const product = {
            ...
            r49Points: 0,
            cosmeticPoints: 0,
            cosmeticSerialNumbers: new Set<string>(),
            inventoryPoints: 0,
            slowMovingPoints: 0,
        }
    ...
    for (const accumulator of monthlyMap.values()) {
        ...
        for (const customerBucket of accumulator.cosmeticCustomerBuckets.values()) {
            ...
            if (product) {
                ...
                // Collect matching SNs from the lookup map (display only, no effect on points)
                const snSet = cosmeticSnByMaterial.get(assignedMaterial.materialKey)
                if (snSet) {
                    for (const sn of snSet) {
                        product.cosmeticSerialNumbers.add(sn)
                    }
                }
            }
        }
        ...
        .map((item) => ({
            ...
            soldProducts: Array.from(item.soldProducts.values())
                .map((product) => ({
                    ...
                    cosmeticSerialNumbers: Array.from(product.cosmeticSerialNumbers).sort((a, b) => a.localeCompare(b)),
