...
export async function getOrgChartData() {
  ...
  const employeeRows = await db
    .select({
      ...
      levelName:
        sql<string>`coalesce(${hrPositions.levelName}, ${employees.levelName}, 'Staff')`.as(
          'level_name'
        ),
      ...
      workLocationId: sql<number | null>`null`.as('work_location_id'),
  ...
  const unassigned = employeeRowsUnique.filter((e) => {
    ...
    if (assignedNode && assignedNode.nodeType === 'department' && e.sectionId) {
      const hasRealSection = nodes.some(
        (n) => n.parentNodeId === assignedNode.id && n.sectionId === e.sectionId
      )
  ...
  const [allDepts, allSects] = await Promise.all([
    deptIds.length
      ? db
          .select()
          .from(hrDepartments)
          .where(inArray(hrDepartments.id, deptIds as number[]))
      : [],
    sectIds.length
      ? db
          .select()
          .from(hrSections)
          .where(inArray(hrSections.id, sectIds as number[]))
      : [],
  ])
  ...
  for (const [, emps] of deptGroup) {
    ...
    const realDeptNode = nodes.find(
      (n) =>
        n.departmentId === first.departmentId && n.nodeType?.toLowerCase().includes('department')
    )
  ...
  for (const employee of employeeRowsUnique) {
    const nodeId =
      employee.orgNodeId && orgNodeIds.has(employee.orgNodeId) ? employee.orgNodeId : null
    if (nodeId) {
...
export async function updateOrgNodeParent(nodeId: number, newParentId: number | null) {
  try {
    if (await wouldCreateCircularRef(nodeId, newParentId)) {
      return {
        success: false,
        message: 'Circular reference detected. Node tidak bisa menjadi child dari descendantnya.',
      }
    }
  ...
  try {
    ...
    const [newNode] = await db
      ...
      .values({
        ...
        sectionId:
          data.nodeType.replace(/_virtual$/i, '').toLowerCase() === 'department' ||
          data.nodeType.replace(/_virtual$/i, '').toLowerCase() === 'company' ||
          data.nodeType
            .replace(/_virtual$/i, '')
            .toLowerCase()
            .includes('bod')
    ...
    const employeeIds = Array.from(
      new Set(data.employeeIds.filter((id) => Number.isFinite(id) && id > 0))
    )
    ...
    return {
      success: true,
      message: 'Node virtual berhasil dijadikan node real dan dipindahkan.',
      nodeId: newNode.id,
    }
  } catch (error) {
...
export async function materializeAndMoveOrgVirtualNode(data: {
  ...
  try {
    ...
    if (data.target) {
      const targetResult = await createRealOrgNodeFromVirtual(
        data.target,
        data.fallbackParentNodeId
      )
...
async function createRealOrgNodeFromVirtual(
  node: VirtualOrgNodePayload,
  parentNodeId: number | null
) {
  const existingNode =
    node.id > 0
      ? await db
          .select({ id: hrOrgNodes.id })
          .from(hrOrgNodes)
          .where(eq(hrOrgNodes.id, node.id))
          .limit(1)
      : []
  ...
  const [newNode] = await db
    ...
    .values({
      ...
      sectionId:
        node.nodeType.replace(/_virtual$/i, '').toLowerCase() === 'department' ||
        node.nodeType.replace(/_virtual$/i, '').toLowerCase() === 'company' ||
        node.nodeType
          .replace(/_virtual$/i, '')
          .toLowerCase()
          .includes('bod')
  ...
  const actualPath = parentNodeId
    ? (await calculateHierarchy(newNode.id, parentNodeId)).path
    : `/${newNode.id}/`
  await db.update(hrOrgNodes).set({ pathText: actualPath }).where(eq(hrOrgNodes.id, newNode.id))

  const employeeIds = Array.from(
    new Set(node.employeeIds.filter((id) => Number.isFinite(id) && id > 0))
  )
  if (employeeIds.length > 0) {
    const updatedEmployees = await db
      .update(employees)
  ...
  try {
    ...
    if (!deptId) {
      const [matchedDept] = await db
        ...
        .where(
          and(
            eq(hrDepartments.isActive, true),
            sql`lower(trim(${hrDepartments.name})) = lower(trim(${data.name}))`
          )
        )
        .limit(1)
      if (matchedDept) {
    ...
    if (!sectId && (data.nodeType === 'section' || data.nodeType === 'unit')) {
      const [matchedSect] = await db
        ...
        .where(
          and(
            eq(hrSections.isActive, true),
            sql`lower(trim(${hrSections.name})) = lower(trim(${data.name}))`
          )
        )
        .limit(1)
      if (matchedSect) {
...
export async function updateOrgNode(
  ...
  try {
    ...
    if (deptId === undefined && nameToMap) {
      const [matchedDept] = await db
        ...
        .where(
          and(
            eq(hrDepartments.isActive, true),
            sql`lower(trim(${hrDepartments.name})) = lower(trim(${nameToMap}))`
          )
        )
        .limit(1)
      if (matchedDept) {
    ...
    if (
      sectId === undefined &&
      nameToMap &&
      (nodeTypeToMap === 'section' || nodeTypeToMap === 'unit')
    ) {
      const [matchedSect] = await db
        .select({ id: hrSections.id, departmentId: hrSections.departmentId })
        .from(hrSections)
        .where(
          and(
            eq(hrSections.isActive, true),
            sql`lower(trim(${hrSections.name})) = lower(trim(${nameToMap}))`
          )
        )
        .limit(1)
      if (matchedSect) {
    ...
    if (leaderEmployeeId) {
      ...
      const scored = rawPositions
        ...
        .sort(
          (a, b) =>
            ...
            a.position.rankName.localeCompare(b.position.rankName, 'id-ID')
        )
      ...
      const employeeUpdate = {
        ...
        ...(nodeDefaults?.siteId !== null && nodeDefaults?.siteId !== undefined
...
export async function deleteOrgNode(id: number) {
  try {
    ...
    if (children.length > 0) {
      return {
        success: false,
        message: 'Node memiliki child nodes. Hapus atau pindahkan child nodes terlebih dahulu.',
      }
    }
    ...
    if (assignedEmployees.length > 0) {
      return {
        success: false,
        message: 'Node memiliki karyawan assigned. Pindahkan karyawan terlebih dahulu.',
      }
    }
...
export async function updateOrgChartEmployeeAssignment(employeeId: number, orgNodeId: number) {
  try {
    ...
    return {
      success: true,
      message: 'Karyawan berhasil dipindahkan dan metadata attendance disync.',
    }
  } catch (error) {
...
export async function updateOrgChartEmployeeNodeHead(
  ...
  try {
    ...
    return {
      success: true,
      message: 'Karyawan berhasil dijadikan head node dan disync ke User Management.',
    }
  } catch (error) {
...
async function resolveLegacySiteId(hrSiteName?: string | null, workLocationName?: string | null) {
  ...
  for (const candidate of candidates) {
    const partial = keyedRows.find((site) =>
      site.keys.some(
        (key) => key.length > 2 && (candidate.includes(key) || key.includes(candidate))
      )
    )
...
async function syncOrgChartEmployeeToOperationalEmployee(employee: SyncedOrgChartEmployee) {
  const [[department], [section], [position], [site]] = await Promise.all([
    ...
    employee.positionId
      ? db
          .select({ rankName: hrPositions.rankName, levelName: hrPositions.levelName })
          .from(hrPositions)
          .where(eq(hrPositions.id, employee.positionId))
          .limit(1)
      : Promise.resolve([]),
    employee.siteId
...
export async function createOrgChartEmployee(data: {
  ...
  try {
    ...
    return {
      success: true,
      message: 'Orang berhasil dibuat di struktur organisasi.',
...
export async function updateOrgChartEmployeeProfile(
  ...
  try {
    ...
    return {
      success: true,
      message: 'Profil karyawan berhasil diupdate dan disync ke User Management.',
    }
  } catch (error) {
...
export async function getOrgNodeReferenceData() {
  const [departments, sections, sitesData, workLocations, rawPositions, levelStaffs] =
    await Promise.all([
      db
      ...
      db
        .select({ id: sites.id, name: sites.location })
        .from(sites)
        .where(eq(sites.isActive, true))
        .orderBy(asc(sites.location)),
      db
        .select({
          id: hrPositions.id,
          rankName: hrPositions.rankName,
        ...
        .from(hrPositions)
        .where(eq(hrPositions.isActive, true))
        .orderBy(asc(hrPositions.levelName), asc(hrPositions.rankName)),
      db
        .select({ name: masterLevelStaff.name, sortOrder: masterLevelStaff.sortOrder })
        .from(masterLevelStaff)
        .where(eq(masterLevelStaff.isActive, true))
        .orderBy(asc(masterLevelStaff.sortOrder)),
    ])
