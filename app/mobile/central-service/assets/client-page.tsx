...
export function MobileAssetsClientPage({
  ...
  return (
    <div className="space-y-4 pb-24 font-sans max-w-lg mx-auto px-1.5">
      ...
      <Dialog open={Boolean(attachmentModalAsset)} onOpenChange={() => setAttachmentModalAsset(null)}>
        <DialogContent className="max-w-lg rounded-3xl p-5">
          <DialogHeader>
            <DialogTitle className="text-sm font-black text-[#003461]">Attachment Asset</DialogTitle>
            <DialogDescription className="text-xs text-slate-500">
              {attachmentModalAsset?.description}
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-3 mt-2 max-h-[60vh] overflow-y-auto">
            {attachmentModalAsset?.attachments?.map((att, idx) => {
              ...
              return (
                <div key={idx} className="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-[#003461] truncate max-w-[220px]">
                      {att.fileName}
                    </span>
                    <a
                      href={url}
                      target="_blank"
                      rel="noreferrer"
                      className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg flex items-center gap-1"
                    >
                  ...
                  {isImg ? (
                    <img src={url} alt={att.fileName} className="w-full max-h-48 object-contain rounded-xl bg-white border" />
                  ) : (
                    <iframe title={att.fileName} src={url} className="w-full h-48 rounded-xl bg-white border" />
                  )}
                </div>
              );
            })}
