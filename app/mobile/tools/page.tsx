...
export default async function MobileToolsPage() {
  ...
  const requests = allRequests.filter(r => (r.requestCategory || "APD") === "TOOLS");
  ...
  return (
    <div className="space-y-4 pb-6">
      {/* Header */}
      <section className="rounded-xl bg-gradient-to-br from-emerald-600 to-emerald-800 p-5 text-white">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-wider text-emerald-200">HSE &bull; REQUEST TOOLS</p>
            <h1 className="mt-1 text-xl font-bold tracking-tight">Request Tools</h1>
          </div>
          <Link
            prefetch={false}
            href="/mobile/tools/new"
            className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-white/10 active:scale-95 transition-transform"
            aria-label="Ajukan Tools"
          >
            <Plus className="size-5 text-emerald-200" />
          </Link>
        </div>
        ...
        <div className="mt-4 rounded-xl bg-white/10 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <span className="inline-flex items-center gap-1 rounded-md bg-white/15 px-2 py-0.5 text-[10px] font-medium text-emerald-100">
                <Wrench className="size-3" /> Tools Request
              </span>
              <p className="mt-3 text-3xl font-bold leading-none">{requests.length}</p>
              <p className="mt-2 text-sm text-emerald-200">
                Total request tools Anda
              </p>
            </div>
            <Link
              prefetch={false}
              href="/mobile/tools/new"
              className="inline-flex h-10 items-center gap-1.5 rounded-xl bg-white px-4 text-xs font-semibold text-emerald-700 active:scale-95 transition-transform"
            >
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            <div className="rounded-lg bg-white/10 px-3 py-2 text-center">
              <p className="text-[10px] font-medium text-emerald-200">Menunggu Approval</p>
              <p className="mt-0.5 text-base font-bold text-amber-300">{pendingCount}</p>
            </div>
            <div className="rounded-lg bg-white/10 px-3 py-2 text-center">
              <p className="text-[10px] font-medium text-emerald-200">Proses Order</p>
            ...
            <div className="rounded-lg bg-white/10 px-2 py-2 text-center">
              <p className="text-[10px] font-medium text-emerald-200">Complete</p>
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-[11px] font-medium uppercase tracking-wider text-gray-500">Riwayat Pengajuan Tools</h2>
          <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700">{requests.length} tiket</span>
        </div>
        ...
        {requests.length > 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        <h2 className="text-sm font-semibold leading-tight text-gray-900">Request Tools</h2>
                      </div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-8 text-center text-sm text-gray-500">Belum ada riwayat pengajuan tools.</div>
        )}
      </section>
    </div>
  );
