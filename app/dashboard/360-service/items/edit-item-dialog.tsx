...
import { updateItem } from "@/app/actions/service360"
import { Edit } from "lucide-react"

export function EditItemDialog({ item, siteList }: { item: any, siteList: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (name && category && price) {
        await updateItem(item.id, { name, category, price, siteId, jobTitle })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>Update the details of this item or service.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div>
            ...
            <Input name="name" required defaultValue={item.name} placeholder="Item/Service Name" className="mt-1.5" />
          </div>
          <div>
            <label className="text-sm font-medium">Price</label>
            <Input name="price" type="number" step="0.01" required defaultValue={Math.floor(Number(item.price))} placeholder="0.00" className="mt-1.5" />
          </div>
