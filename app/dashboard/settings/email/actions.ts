...
const attendanceNotificationSchema = z.object({
  slaRemindersEnabled: z.preprocess(
...
export async function saveAttendanceNotificationConfigAction(
  ...
  if (!access.canEdit) {
    return {
      status: "error",
      message: "Akses ditolak: Anda tidak memiliki izin mengubah konfigurasi notifikasi.",
    };
  }
  ...
  if (!parsed.success) {
    return {
      status: "error",
      message: parsed.error.issues[0]?.message ?? "Konfigurasi reminder SLA belum valid.",
    };
  }
  ...
  try {
    ...
    const values = {
      slaRemindersEnabled: parsed.data.slaRemindersEnabled,
      updatedAt: new Date(),
    };
    ...
    return {
      status: "success",
      message: "Pengaturan reminder SLA izin absensi berhasil disimpan.",
    };
  } catch (error) {
    return {
      status: "error",
      message: error instanceof Error ? error.message : "Gagal menyimpan reminder SLA izin absensi.",
    };
  }
}
...
export async function saveMaterialToolsNotificationConfigAction(
  ...
  if (!parsed.success) return { status: 'error', message: 'Konfigurasi penerima Material & Tools belum valid.' };
  try {
    const [existing] = await db.select({ id: materialToolsNotificationConfig.id }).from(materialToolsNotificationConfig).limit(1);
    const values = { ...parsed.data, updatedAt: new Date() };
    if (existing) await db.update(materialToolsNotificationConfig).set(values).where(eq(materialToolsNotificationConfig.id, existing.id));
    else await db.insert(materialToolsNotificationConfig).values(values);
    revalidatePath('/dashboard/settings/email');
    return { status: 'success', message: 'Pengaturan Material & Tools berhasil disimpan.' };
  } catch (error) {
    return { status: 'error', message: error instanceof Error ? error.message : 'Gagal menyimpan pengaturan Material & Tools.' };
  }
}
