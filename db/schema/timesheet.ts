...
export const timesheetPayrollSnapshotItems = pgTable(
  ...
  (table) => ({
    snapshotEmployeeDayUnique: uniqueIndex("hero_timesheet_payroll_snapshot_items_snapshot_employee_day_uidx").on(
      table.snapshotId,
      table.employeeId,
      table.day
    ),
...
export const timesheetAttendanceRealOverrides = pgTable(
  "hero_timesheet_attendance_real_overrides",
  {
    ...
    source: text("source").notNull().default("manual"),
    importPreviewId: integer("import_preview_id").references(() => timesheetAttendanceImportPreviews.id, {
      onDelete: "set null",
    }),
