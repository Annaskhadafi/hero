...
async function main() {
  const legacy = await db
    ...
    .from(employees)
    .where(sql`CAST(${employees.employeeSn} AS INTEGER) < 1000 AND ${employees.employeeSn} ~ '^\\d+$'`)
    .orderBy(sql`CAST(${employees.employeeSn} AS INTEGER)`);

  console.log("=== employees with SN < 1000 ===");
  ...
  const hr = await db
    ...
    .from(hrEmployees)
    .where(sql`CAST(${hrEmployees.employeeId} AS INTEGER) < 1000 AND ${hrEmployees.employeeId} ~ '^\\d+$'`)
    .orderBy(sql`CAST(${hrEmployees.employeeId} AS INTEGER)`);

  console.log("\n=== hr_employees with SN < 1000 ===");
