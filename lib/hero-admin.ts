...
const RAW_SIDEBAR_MENU_SEEDS = [
  ...
  },
  {
    ...
    url: '/dashboard/apd/inventory',
    iconName: 'database',
    resource: 'apd_inventory',
    sortOrder: 2,
    isVisible: false,
    openInNewTab: false,
  },
  ...
  },
  {
    ...
    section: 'Central Service',
    groupLabel: 'Repair & Retread',
    title: 'WIP Dashboard V2',
    url: '/dashboard/repair-retread/wip-repair/dashboard-v2',
    iconName: 'chart-bar',
    resource: 'wip_repair_dashboard_v2',
    sortOrder: 3,
    isVisible: true,
    openInNewTab: false,
  },
...
async function ensureHeroGovernanceTables() {
  ...
  await db.execute(sql`
    create table if not exists hero_cs_forecast_daily_report_config (
      ...
      recipient_emails text not null default '',
      cc_emails text not null default '',
      send_times text not null default '08:00',
...
export async function getCsForecastDailyReportConfigData() {
  await ensureHeroGovernanceSeedData()
...
export async function getActiveEmployeesForSelect() {
  // User Management (hero_employees) — only rows with usable email
  const rows = await db
    .select({
      id: employees.id,
      name: employees.name,
