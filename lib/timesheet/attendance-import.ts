...
export const attendanceImportRawRowSchema = z.object({
  ...
  status: z
    .enum(['present', 'empty', 'sick', 'leave', 'absent', 'off', 'standby', 'field_break'])
    .default('present'),
...
export type AttendanceValidationFlag =
  | 'missing-clock-in'
  | 'missing-clock-out'
  | 'late'
  | 'early-out'
...
export type AttendanceValidationSummary = Record<
  | 'matched'
  | 'skipped'
  | 'conflict'
  | 'duplicate'
...
function buildEmployeeMatcher(
  ...
  const byAliasSn = new Map(
    matchers.filter((item) => item.aliasSn).map((item) => [item.aliasSn, item.employee])
  )
  const byAliasName = new Map(
    matchers.filter((item) => item.aliasName).map((item) => [item.aliasName, item.employee])
  )
  const fuse = new Fuse(matchers, {
    keys: ['normalizedName', 'normalizedSn', 'aliasName', 'aliasSn'],
    threshold: 0.32,
    ignoreLocation: true,
...
function matchEmployee(
  input: { snKey: string; nameKey: string },
  matcher: ReturnType<typeof buildEmployeeMatcher>
) {
  if (input.snKey && matcher.bySn.has(input.snKey))
    return { employee: matcher.bySn.get(input.snKey)!, matchMethod: 'sn' as const, matchScore: 0 }
  if (input.nameKey && matcher.byName.has(input.nameKey))
    return {
      employee: matcher.byName.get(input.nameKey)!,
      matchMethod: 'name' as const,
      matchScore: 0,
    }
  if (input.snKey && matcher.byAliasSn.has(input.snKey))
    return {
      employee: matcher.byAliasSn.get(input.snKey)!,
      matchMethod: 'alias' as const,
      matchScore: 0,
    }
  if (input.nameKey && matcher.byAliasName.has(input.nameKey))
    return {
      employee: matcher.byAliasName.get(input.nameKey)!,
      matchMethod: 'alias' as const,
      matchScore: 0,
    }
  ...
  const result = matcher.fuse
    .search(query)
    .sort(
      (left, right) =>
        (left.score ?? 1) - (right.score ?? 1) || left.item.siteRank - right.item.siteRank
    )[0]
  if (!result || (result.score ?? 1) > 0.32)
    return { employee: null, matchMethod: 'none' as const, matchScore: result?.score }
  return {
    employee: result.item.employee,
    matchMethod: 'fuse' as const,
...
function buildValidationFlags(
  row: { status: string; clockIn: string; clockOut: string },
  match: { matchMethod: string; matchScore?: number },
...
export function buildAttendanceImportPreview(params: {
  ...
  const previewRows: AttendancePreviewRow[] = adjustedRows.map((raw, index) => {
    const row = {
      ...
      clockOut: normalizeTime(raw.clockOut),
    }
    ...
    if (
      ...
      row.status === 'present' &&
      scheduleCode &&
      blockedScheduleCodes.has(scheduleCode)
    ) {
      conflicts.push({
    ...
    return {
      ...
      matchWarning: validation.flags.includes('low-confidence')
  ...
  return {
    ...
    conflicts,
    validationSummary,
    matchedCount: previewRows.filter((row) => row.employeeId && !row.crossSite && !row.duplicate)
      .length,
    unmatchedCount: previewRows.filter((row) => row.unmatched || row.crossSite || row.duplicate)
      .length,
    cellCount: previewRows.filter((row) => row.employeeId && !row.crossSite && !row.duplicate)
      .length,
