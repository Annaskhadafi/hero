...
const globalState = globalThis as unknown as { __heroNotificationInfrastructureReady?: boolean };

export async function ensureNotificationInfrastructure() {
  if (globalState.__heroNotificationInfrastructureReady) {
    return;
  }
  ...
  notificationInfrastructurePromise = (async () => {
    try {
      ...
      globalState.__heroNotificationInfrastructureReady = true;
    } catch (error) {
      notificationInfrastructurePromise = null;
      console.warn('[ensureNotificationInfrastructure] Non-fatal init warning:', error);
    }
  })();
