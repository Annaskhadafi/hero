...
Silakan masuk ke dashboard untuk membalas pesan.`,
  ...
  },
  {
    ...
    description: 'Notifikasi email ketika HR meneruskan tiket pengaduan ke Section dan PIC.',
    variables: ['ticketNumber', 'employeeName', 'category', 'sectionName', 'picName', 'note', 'sessionId'],
    sampleValues: { ticketNumber: 'HR-2026-000001', employeeName: 'Andi Kusuma', category: 'Konflik Pekerjaan', sectionName: 'Operasional', picName: 'Budi Santoso', note: 'Mohon ditindaklanjuti.', sessionId: '12' },
  },
  {
...
Tim Human Capital`,
  ...
  {
    name: 'Material & Tools Request Submitted',
    ...
    htmlContent: 'Halo {{approverName}},<br><br>Karyawan <b>{{employeeName}}</b> (Section: {{sectionName}}) telah mengajukan permohonan <b>{{requestType}}</b> dengan nomor tiket <b>{{requestNumber}}</b> yang memerlukan persetujuan Anda sebagai Section Head.<br><br>Silakan login ke dashboard HERO untuk melakukan review dan persetujuan.<br><br>CC: Muhammad Taufik Akbar<br>Terima kasih.',
    ...
    variables: ['employeeName', 'requestNumber', 'approverName', 'requestType', 'sectionName'],
    sampleValues: {
      employeeName: 'Budi Santoso',
  ...
  {
    name: 'Material & Tools Request Approved',
    ...
    htmlContent: 'Halo {{employeeName}},<br><br>Permohonan <b>{{requestType}}</b> Anda dengan nomor tiket <b>{{requestNumber}}</b> telah <b>DISETUJUI</b> oleh Section Head ({{approverName}}).<br><br>Notifikasi ini juga telah diteruskan ke Muhammad Taufik Akbar.<br><br>Terima kasih.',
  ...
  },
  {
    ...
    recipientScope: 'approver',
    ccEmail: '',
    subject: '[Form WO] Pengajuan Work Order Baru: {{noPengajuan}} - {{customer}} ({{jobType}})',
    htmlContent: '<div style="font-family:\'Segoe UI\',Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8fafc;padding:24px;border-radius:12px;border:1px solid #e2e8f0;"><div style="background:#2563eb;padding:16px 20px;border-radius:8px 8px 0 0;"><h2 style="color:#ffffff;margin:0;font-size:18px;font-weight:700;">HERO &bull; Form Work Order</h2><p style="color:#bfdbfe;margin:4px 0 0;font-size:12px;">Persetujuan Dokumen Form WO</p></div><div style="background:#ffffff;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;border-top:none;"><p style="font-size:14px;color:#334155;line-height:1.6;margin:0 0 16px;">Yth. <b>{{approverName}}</b>,<br>Pengajuan Form Work Order berikut membutuhkan peninjauan dan persetujuan Anda:</p><table style="width:100%;border-collapse:collapse;font-size:13px;color:#334155;margin-bottom:20px;"><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;width:130px;color:#64748b;">No. Pengajuan</td><td style="padding:8px 0;font-weight:700;color:#0f172a;">{{noPengajuan}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Pemohon</td><td style="padding:8px 0;">{{pemohon}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Customer / Site</td><td style="padding:8px 0;">{{customer}} / {{site}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Jenis Pekerjaan</td><td style="padding:8px 0;">{{jobType}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Serial Number</td><td style="padding:8px 0;">{{tireSn}} ({{brand}} {{size}})</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Total Biaya</td><td style="padding:8px 0;font-weight:700;color:#059669;">{{totalAmount}}</td></tr><tr><td style="padding:8px 0;font-weight:600;color:#64748b;vertical-align:top;">Catatan</td><td style="padding:8px 0;">{{catatanPengajuan}}</td></tr></table><div style="text-align:center;margin:28px 0 16px 0;"><a href="{{approvalLink}}" style="background-color:#2563eb;color:#ffffff;padding:12px 28px;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;display:inline-block;box-shadow:0 2px 4px rgba(37,99,235,0.25);">Buka Inbox Approval &amp; Review Dokumen &rarr;</a></div><p style="font-size:11px;color:#94a3b8;text-align:center;margin:16px 0 0;">Jika tombol di atas tidak dapat diklik, buka tautan berikut:<br><a href="{{approvalLink}}" style="color:#2563eb;">{{approvalLink}}</a></p></div></div>',
    textContent: 'Yth. {{approverName}},\n\nPengajuan Work Order (Form WO) {{noPengajuan}} untuk customer {{customer}} ({{jobType}}) memerlukan persetujuan Anda.\n\nNo. Pengajuan: {{noPengajuan}}\nPemohon: {{pemohon}}\nCustomer / Site: {{customer}} / {{site}}\nJenis Pekerjaan: {{jobType}}\nSerial Number: {{tireSn}} ({{brand}} {{size}})\nTotal Biaya: {{totalAmount}}\nCatatan: {{catatanPengajuan}}\n\nBuka Inbox Approval:\n{{approvalLink}}\n\nTerima kasih.',
    description: 'Notifikasi pengajuan Work Order (Form WO) baru yang memerlukan approval.',
    variables: ['approverName', 'noPengajuan', 'pemohon', 'customer', 'site', 'jobType', 'tireSn', 'brand', 'size', 'totalAmount', 'catatanPengajuan', 'approvalLink'],
    sampleValues: {
      approverName: 'Hendra Wijaya',
      noPengajuan: 'FRMWO/26/08/0042',
      pemohon: 'Joko Susilo',
      customer: 'PT Kaltim Prima Coal (KPC)',
      site: 'Site Sangatta',
      jobType: 'Major Repair 53/80R63',
      tireSn: 'SN-KPC-2026-9812',
      brand: 'Bridgestone',
      size: '53/80R63',
      totalAmount: 'Rp 45.000.000',
      catatanPengajuan: 'Perbaikan sela dinding ban akibat batu pit.',
      approvalLink: 'https://hero.chitraparatama.co.id/dashboard/approval',
    },
  },
  {
    name: 'Form WO Approved & Work Order Issued',
    templateCode: 'form_wo_status_approved',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'requester,admin',
    ccEmail: '',
    subject: '[Form WO Disetujui] Pengajuan WO {{noPengajuan}} Disetujui (WO Terbit: {{noWoTerbit}})',
    htmlContent: 'Yth. {{pemohon}}, Pengajuan Form WO {{noPengajuan}} telah DISETUJUI dan Work Order resmi diterbitkan: {{noWoTerbit}}.',
    textContent: 'Yth. {{pemohon}},\n\nPengajuan Form WO {{noPengajuan}} telah DISETUJUI.\nNo. WO Terbit: {{noWoTerbit}}\nCustomer / Site: {{customer}} / {{site}}\nJenis Pekerjaan: {{jobType}}\nTotal Amount: {{totalAmount}}\n\nPekerjaan dapat segera dilaksanakan.',
    description: 'Notifikasi saat Form WO disetujui dan nomor WO resmi diterbitkan.',
    variables: ['pemohon', 'noPengajuan', 'noWoTerbit', 'customer', 'site', 'jobType', 'totalAmount'],
    sampleValues: {
      pemohon: 'Joko Susilo',
      noPengajuan: 'FRMWO/26/08/0042',
      noWoTerbit: 'WO-REPAIR-2026-0891',
      customer: 'PT Kaltim Prima Coal (KPC)',
      site: 'Site Sangatta',
      jobType: 'Major Repair 53/80R63',
      totalAmount: 'Rp 45.000.000',
  ...
  {
    name: 'Form WO Rejected',
    templateCode: 'form_wo_status_rejected',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'requester',
    ccEmail: '',
    subject: '[Form WO Ditolak] Pengajuan WO {{noPengajuan}} Ditolak',
    htmlContent: 'Yth. {{pemohon}}, Pengajuan Form WO {{noPengajuan}} telah DITOLAK.',
    textContent: 'Yth. {{pemohon}},\n\nPengajuan Form WO {{noPengajuan}} telah DITOLAK.\nCatatan Penolakan: {{catatanPengajuan}}',
    description: 'Notifikasi penolakan pengajuan Form WO.',
    variables: ['pemohon', 'noPengajuan', 'catatanPengajuan'],
    sampleValues: {
      pemohon: 'Joko Susilo',
      noPengajuan: 'FRMWO/26/08/0042',
      catatanPengajuan: 'Kerusakan ban melebihi batas toleransi repair safety.',
    },
  },
  {
    name: 'Form WO Reverted / Perlu Revisi',
    templateCode: 'form_wo_status_reverted',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'requester',
    ccEmail: '',
    subject: '[Form WO Perlu Revisi / Revert] Pengajuan WO {{noPengajuan}} Perlu Diperbaiki',
    htmlContent: 'Yth. {{pemohon}}, Pengajuan Form WO {{noPengajuan}} telah dikembalikan oleh approver untuk dilakukan REVISI (Revert). Catatan: {{catatanRevisi}}',
    textContent: 'Yth. {{pemohon}},\n\nPengajuan Form WO {{noPengajuan}} telah dikembalikan oleh approver untuk dilakukan REVISI (Revert).\n\nCatatan Revisi:\n{{catatanRevisi}}\n\nSilakan perbaiki data form di sistem HERO tanpa perlu mengajukan dari awal:\n{{revisiLink}}',
    description: 'Notifikasi saat Form WO di-revert oleh approver dan dikembalikan ke pemohon untuk revisi.',
    variables: ['pemohon', 'noPengajuan', 'catatanRevisi', 'revisiLink'],
    sampleValues: {
      pemohon: 'Joko Susilo',
      noPengajuan: 'FRMWO/26/08/0042',
      catatanRevisi: 'Mohon update nomor PO dan sesuaikan serial number ban di baris 2.',
      revisiLink: 'https://hero.chitraparatama.co.id/dashboard/repair-retread/form-wo',
    },
  },
  {
    name: 'Form WO Billing Approved',
    templateCode: 'form_wo_billing_approved',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'requester',
    ccEmail: '',
    subject: '[Form WO Billing Disetujui] Pengajuan WO {{noPengajuan}} Telah Disetujui Team Billing (Step 4)',
    htmlContent: 'Yth. {{pemohon}}, Pengajuan Form WO {{noPengajuan}} telah disetujui oleh Team Billing (Step 4) dan diteruskan ke Inventory & Warehouse Management SPV (Step 5) untuk persetujuan akhir.',
    textContent: 'Yth. {{pemohon}},\n\nPengajuan Form WO {{noPengajuan}} telah disetujui oleh Team Billing (Step 4).\nSaat ini pengajuan diteruskan ke tahap persetujuan akhir oleh Inventory & Warehouse Management SPV (Step 5).\n\nCustomer: {{customer}}\nSite: {{site}}\nPekerjaan: {{jobType}}\nTotal: {{totalAmount}}\n\nTerima kasih.',
    description: 'Notifikasi ke pemohon saat Team Billing telah menyetujui Form WO (Step 4).',
    variables: ['pemohon', 'noPengajuan', 'customer', 'site', 'jobType', 'totalAmount'],
    sampleValues: {
      pemohon: 'Joko Susilo',
      noPengajuan: 'FRMWO/26/08/0042',
      customer: 'PT Kaltim Prima Coal (KPC)',
  ...
  {
    name: 'Form WO Ready for WO Number (To Billing)',
    templateCode: 'form_wo_ready_for_wo_number',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'billing,admin',
    ccEmail: '',
    subject: '[Form WO Siap Terbit] Silakan Terbitkan Nomor WO untuk: {{noPengajuan}}',
    htmlContent: '<div style="font-family:\'Segoe UI\',Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8fafc;padding:24px;border-radius:12px;border:1px solid #e2e8f0;"><div style="background:#059669;padding:16px 20px;border-radius:8px 8px 0 0;"><h2 style="color:#ffffff;margin:0;font-size:18px;font-weight:700;">HERO &bull; Form Work Order</h2><p style="color:#d1fae5;margin:4px 0 0;font-size:12px;">Persetujuan Final Selesai &bull; Siap Terbit Nomor WO</p></div><div style="background:#ffffff;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;border-top:none;"><p style="font-size:14px;color:#334155;line-height:1.6;margin:0 0 16px;">Halo <b>Billing Team</b>,<br>Form Work Order nomor <b>{{noPengajuan}}</b> telah <b>disetujui lengkap</b>. Silakan terbitkan dan isi Nomor WO resmi pada sistem:</p><table style="width:100%;border-collapse:collapse;font-size:13px;color:#334155;margin-bottom:20px;"><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:6px 0;font-weight:600;width:130px;color:#64748b;">No. Pengajuan</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">{{noPengajuan}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:6px 0;font-weight:600;color:#64748b;">Pemohon</td><td style="padding:6px 0;">{{pemohon}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:6px 0;font-weight:600;color:#64748b;">Customer / Site</td><td style="padding:6px 0;">{{customer}} / {{site}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:6px 0;font-weight:600;color:#64748b;">Jenis Pekerjaan</td><td style="padding:6px 0;">{{jobType}}</td></tr><tr><td style="padding:6px 0;font-weight:600;color:#64748b;">Total Amount</td><td style="padding:6px 0;font-weight:700;color:#059669;">{{totalAmount}}</td></tr></table><div style="text-align:center;margin:28px 0 16px 0;"><a href="{{inputWoLink}}" style="background-color:#059669;color:#ffffff;padding:12px 28px;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;display:inline-block;box-shadow:0 2px 4px rgba(5,150,105,0.25);">Isi Nomor WO Resmi Sekarang &rarr;</a></div></div></div>',
    textContent: 'Yth. Billing Team,\n\nForm WO nomor {{noPengajuan}} untuk {{customer}} telah disetujui lengkap.\n\nSilakan isi Nomor WO Resmi pada sistem HERO:\n{{inputWoLink}}\n\nTerima kasih.',
    description: 'Notifikasi ke Billing Team untuk mengisi Nomor WO setelah seluruh tahapan approval Form WO selesai.',
    variables: ['noPengajuan', 'pemohon', 'customer', 'site', 'jobType', 'totalAmount', 'inputWoLink'],
    sampleValues: {
      noPengajuan: 'FRMWO/26/08/0042',
      pemohon: 'Joko Susilo',
      customer: 'PT Kaltim Prima Coal (KPC)',
  ...
  {
    name: 'Form WO Final Completed (With PDF)',
    templateCode: 'form_wo_final_completed_pdf',
    templateType: 'Notification',
    deliveryChannel: 'email,bell',
    recipientScope: 'requester,admin',
    ccEmail: '',
    subject: '[Form WO Resmi Terbit] {{noPengajuan}} - WO CP: {{noWoTerbit}} (No. PO: {{noPo}})',
    htmlContent: '<div style="font-family:\'Segoe UI\',Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8fafc;padding:24px;border-radius:12px;border:1px solid #e2e8f0;"><div style="background:#0f766e;padding:16px 20px;border-radius:8px 8px 0 0;"><h2 style="color:#ffffff;margin:0;font-size:18px;font-weight:700;">HERO &bull; Form Work Order</h2><p style="color:#ccfbf1;margin:4px 0 0;font-size:12px;">Dokumen Resmi Form WO Lengkap Diterbitkan</p></div><div style="background:#ffffff;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e2e8f0;border-top:none;"><p style="font-size:14px;color:#334155;line-height:1.6;margin:0 0 16px;">Yth. <b>Admin CP Site &amp; QC/Leader</b>,<br>Nomor WO Resmi untuk pengajuan <b>{{noPengajuan}}</b> telah diisi dan diterbitkan oleh Team Billing:</p><table style="width:100%;border-collapse:collapse;font-size:13px;color:#334155;margin-bottom:20px;"><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;width:130px;color:#64748b;">No. Pengajuan</td><td style="padding:8px 0;font-weight:700;color:#0f172a;">{{noPengajuan}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;background:#f0fdf4;"><td style="padding:8px 6px;font-weight:700;color:#15803d;">Nomor WO CP</td><td style="padding:8px 6px;font-weight:800;color:#15803d;font-size:14px;">{{noWoTerbit}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;background:#eff6ff;"><td style="padding:8px 6px;font-weight:700;color:#1d4ed8;">Nomor PO</td><td style="padding:8px 6px;font-weight:700;color:#1d4ed8;">{{noPo}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Pemohon</td><td style="padding:8px 0;">{{pemohon}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Customer / Site</td><td style="padding:8px 0;">{{customer}} / {{site}}</td></tr><tr style="border-bottom:1px solid #f1f5f9;"><td style="padding:8px 0;font-weight:600;color:#64748b;">Jenis Pekerjaan</td><td style="padding:8px 0;">{{jobType}}</td></tr><tr><td style="padding:8px 0;font-weight:600;color:#64748b;">Total Biaya</td><td style="padding:8px 0;font-weight:700;color:#059669;">{{totalAmount}}</td></tr></table><div style="background:#f1f5f9;padding:12px 16px;border-radius:6px;font-size:12px;color:#475569;margin-bottom:16px;">📎 <b>Lampiran Dokumen:</b> File PDF Form WO lengkap beserta nomor PO, nomor WO, dan seluruh tanda tangan digital telah dilampirkan pada email ini.</div><div style="text-align:center;margin:20px 0 10px 0;"><a href="https://hero.chitraparatama.co.id/dashboard/repair-retread/form-wo" style="background-color:#0f766e;color:#ffffff;padding:12px 28px;text-decoration:none;border-radius:8px;font-weight:700;font-size:14px;display:inline-block;box-shadow:0 2px 4px rgba(15,118,110,0.25);">Buka Dashboard Form WO &rarr;</a></div></div></div>',
    textContent: 'Yth. Admin CP Site & QC/Leader,\n\nNomor WO resmi untuk pengajuan Form WO {{noPengajuan}} telah diterbitkan oleh Team Billing.\n\nNo. WO CP: {{noWoTerbit}}\nNo. PO: {{noPo}}\nCustomer: {{customer}}\nSite: {{site}}\nTotal Amount: {{totalAmount}}\n\nDokumen PDF Form WO lengkap telah dilampirkan.\n\nTerima kasih.',
    description: 'Notifikasi saat nomor WO resmi diterbitkan dan file PDF Form WO lengkap dikirim ke Admin CP Site dan QC/Leader.',
    variables: ['pemohon', 'noPengajuan', 'noWoTerbit', 'noPo', 'customer', 'site', 'jobType', 'totalAmount'],
    sampleValues: {
      pemohon: 'Joko Susilo',
      noPengajuan: 'FRMWO/26/08/0042',
      noWoTerbit: 'WO-REPAIR-2026-0891',
      noPo: 'PO-KPC-2026-0771',
      customer: 'PT Kaltim Prima Coal (KPC)',
      site: 'Site Sangatta',
      jobType: 'Major Repair 53/80R63',
      totalAmount: 'Rp 45.000.000',
