...
describe('scheduling timesheet workflow', () => {
  ...
  it('exposes scoped live attendance map with GPS activity controls', () => {
    ...
    expect(actionSource).toContain('getLiveAttendanceLocationSettings')
    expect(actionSource).toContain('timesheetSchedulingConfigs.fieldBreakConfig')
    expect(actionSource).toContain('allowOutsideAttendance')
    expect(actionSource).toContain("access.dataScope !== 'site'")
    expect(actionSource).toContain('typeof allowOutsideAttendance')
    expect(actionSource).toContain("row.geoLatitude.trim() ? Number(row.geoLatitude) : Number.NaN")
    expect(leafletSource).toContain('tile.openstreetmap.org')
    expect(leafletSource).toContain('canEdit')
    expect(leafletSource).toContain('locationOnly')
    expect(leafletSource).toContain('!locationOnly && records.map')
    expect(leafletSource).toContain('focusSite')
    expect(leafletSource).toContain('map.flyTo([focusSite.latitude, focusSite.longitude]')
    expect(leafletSource).toContain('MapCenterReporter')
    expect(leafletSource).toContain("map.on('moveend', reportCenter)")
    expect(mapSource).toContain('30000')
    expect(mapSource).toContain('Pengaturan Lokasi')
    expect(mapSource).toContain('Lihat lokasi saja')
    expect(mapSource).toContain('Filter lokasi')
    expect(mapSource).toContain('unconfiguredSites')
    expect(mapSource).toContain('focusedSite')
    expect(mapSource).toContain('mapCenter?.latitude')
    expect(mapSource).toContain('Keterlambatan / GPS Off')
    expect(mapSource).toContain('getAttendanceExceptions')
    expect(mapSource).toContain("kindLabel: Record<AttendanceExceptionKind, string>")
    expect(mapSource).toContain("record.gpsValid === false")
    expect(mapSource).toContain("Kehadiran:\\s*Terlambat")
