        ...
        {filteredCount > 0 ? (
          <div className="flex items-center justify-end gap-2">
            <span className="text-muted-foreground text-xs font-medium tabular-nums">
              Halaman {Math.min(pageIndex + 1, totalPages)} / {totalPages}
            </span>
            <Button
            ...
            >
              <IconChevronLeft className="size-4" /> <span>Sebelumnya</span>
            </Button>
            <Button
              type="button" variant="outline" size="sm"
              ...
              className="border-border/70 h-8 rounded-lg border bg-white px-2 text-[13px] shadow-none"
            >
              <span>Berikutnya</span> <IconChevronRight className="size-4" />
            </Button>
          </div>
        ) : null}
      </div> : null}
